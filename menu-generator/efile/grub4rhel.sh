...
#		Only for rhel, other OS is under test.
...
function config()
{
	...
	menuentry "$os" --class os {
		...
		linux (tftp)/images/rhel/$os/vmlinuz $repo=http://100.2.36.3/iso/redhat/$os
...
#		read -p "Enter the path: " kspath
		sed -i "15a		linux (tftp)/images/rhel/$os/vmlinuz $repo=http://100.2.36.3/iso/redhat/$os $ks=http://100.2.36.2/ks/rhel/rhel${release}.cfg" boot/grub/grub.cfg
...
function generate()
{
	...
	read -p "Enter the dir to put grub file, leave blank for default(default is /var/lib/tftpboot/grub2.cfg/rhel): " location
if [ "$location" == "" ];then
	cp bootx64.efi /var/lib/tftpboot/grub2.cfg/rhel/${os}.efi
else
